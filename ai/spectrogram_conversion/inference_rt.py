"""
python inference_rt.py --model_name maryann_vatsalsidtom --trg_id 1 --mode online_crossfade
DEBUG=inference_rt python inference_rt.py --model_name maryann_vatsalsidtom --trg_id 1 --mode online_crossfade
"""
import argparse
import math
...
from ai.spectrogram_conversion.params import MAX_INFER_SAMPLES_VC, SEED, sample_rate
...
from ai.spectrogram_conversion.utils.utils import get_conversion_root, get_ordered_data_from_circular_buffer
from ai.spectrogram_conversion.voice_conversion import StudioModelConversionPipeline
...
def periodic_audio_upload_thread(
    audio_in: deque,
    audio_out: deque,
    session_upload_path: str,
    HDW_FRAMES_PER_BUFFER: int,
    MAX_RECORD_SEGMENTS: int,
    time_interval_secs: int = 60,
) -> None:
    segments_path = f"{get_conversion_root()}/segments"
    os.makedirs(segments_path, exist_ok=True)
    n_segments = (sample_rate * time_interval_secs) // HDW_FRAMES_PER_BUFFER
    head = 0
    ...
    while True:
        ...
        # slice segments
        original = get_ordered_data_from_circular_buffer(
            audio_in, buffer_overflow=BUFFER_OVERFLOW, head=head, segment_len=n_segments
        )
        converted = get_ordered_data_from_circular_buffer(
            audio_out, buffer_overflow=BUFFER_OVERFLOW, head=head, segment_len=n_segments
        )
        # update head
        head = (
            (head + n_segments - MAX_RECORD_SEGMENTS + 1)
            if (head + n_segments) >= MAX_RECORD_SEGMENTS
            else (head + n_segments)
        )

        # store to disk
        shutil.rmtree(segments_path)
        os.makedirs(segments_path, exist_ok=True)
        sf.write(f"{segments_path}/{fname_prefix}_original.wav", original, sample_rate)
        sf.write(f"{segments_path}/{fname_prefix}_converted.wav", converted, sample_rate)

        # upload to s3
        upload_directory_to_s3(segments_path, object_prefix=session_upload_path)
...
def run_inference_rt(
    ...
    try:
        ...
        # periodic audio write thread
        if opt.session_upload_path:
            threading.Thread(
                target=periodic_audio_upload_thread,
                daemon=True,
                args=(audio_in, audio_out, opt.session_upload_path, HDW_FRAMES_PER_BUFFER, MAX_RECORD_SEGMENTS),
            ).start()
        ...
        sf.write(
            f"{conversion_root}/original.wav",
            get_ordered_data_from_circular_buffer(audio_in, buffer_overflow=BUFFER_OVERFLOW, head=PACKET_ID),
            sample_rate,
        )
        sf.write(
            f"{conversion_root}/converted.wav",
            get_ordered_data_from_circular_buffer(audio_out, buffer_overflow=BUFFER_OVERFLOW, head=PACKET_ID),
            sample_rate,
        )
